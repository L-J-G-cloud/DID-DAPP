    ...
    <van-dialog v-model:show="show" class="dialog-box" :z-index="100" :showConfirmButton="false"
      ...
      <template #default>
        <div class="head">
          ...
          <img src="@/assets/imgs/identitycasting/close_icon.png" alt="" class="close" @click="show = false" />
        </div>
        <div>
          <p v-for="(item, index) in langs" :key="index"
            class="items_lang d-flex justify-content-between align-items-center" @click="selectLang(index, item)">
            <span>{{ item.text }}</span>

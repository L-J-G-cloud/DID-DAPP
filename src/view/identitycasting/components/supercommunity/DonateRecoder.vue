    ...
    <van-dialog v-model:show="show" class="dialog-box" :closeOnClickOverlay="true">
      ...
      <template #default>
        ...
        <div class="default-box">
          ...
          <!-- 记录列表 -->
          <van-list v-if="recordList.length" v-model:loading="loading" :finished="finished"
            :finished-text="t('no_more')" :loading-text="t('loading')" @load="getRecordListData" :offset="30"
              ...
              <div class="record-item" v-for="(item, index) in recordList" :key="index">
                <div class="item-line">
                  <div class="money F-Bold">
                    <img :src="getBadgeImg(item.type)" alt="" class="badge-img">
                    <span class="F-Regular badge-name">{{ getBadgeName(item.type) }}</span>
                  </div>
                  <div class="d-flex align-items-center">
                    <span class="F-Regular to-address"><img src="@/assets/imgs/identitycasting/address-share.png" alt=""
                        class="address-share-img">{{ getStr(item.to_address, 6, 6) }}</span>
                  </div>
          ...
          <div class="no-record" v-if="!recordList.length && finished">
            ...
            <div class="empty-text">{{ t('no_content') }}</div>
          </div>
...
import { badgeTransferRecord } from "@/api";
import { useI18n } from "vue-i18n";
import { getdata, getResultData, getStr, getBadgeName } from '@/utils'
...
const getRecordListData = async () => {
  const tempApi = badgeTransferRecord;

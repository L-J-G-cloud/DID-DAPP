<template>
                ...
                <div class="record-item" v-for="(item, index) in recordList" :key="index">
                  <div class="item-line">
                    <div class=" F-Bold">
                      <p class="money">
                        <span>
                        {{ formatDecimal(item.amount,4) }}</span>
                        <span class="unit">&nbsp;BLC</span>
                      </p>  
                      
                      <p class="date-time F-SemiBold"> {{ item.create_time ? store.lang === 'en'
                    ? getdata(item.create_time * 1000).langEnStr : getdata(item.create_time * 1000).timeDetail :'---'}}</p>
                    </div>
                    <!-- <div class="describe F-Regular">{{ item.module }}</div> -->
                    <div class="address-box">
                        <img src="@/assets/imgs/identitycasting/address-share.png" alt="">
                        <span>{{getStr(item.to_address,6,4)}}</span>
                    </div>
                  </div>
  ...
  import { nodeTransferRecord, waitReleaseList } from "@/api";
  ...
  const getRecordListData = async () => {
    const tempApi = nodeTransferRecord;

<template>
    <div class="content-box">
        ...
        <div class="verification-data-section" v-if="!valAddress">
            ...
            <div class="data-list">
                <div class="data-item">
                    <span class="label">{{ $t('val_personal_contribution') }}</span>
                    <span class="F-Bold value">{{ verificationData.personalContribution }}</span>
                </div>
                <div class="data-item">
                    <span class="label">{{ $t('val_pool_contribution') }}</span>
                    <span class="F-Bold value">{{ verificationData.poolContribution }}</span>
                </div>
                <div class="data-item">
                    <span class="label">{{ $t('val_pool_reward') }}</span>
                    <span class="F-Bold value">{{ verificationData.poolReward }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="minting-record-section">
            ...
            <div class="record-header">
                ...
                <div class="header-item">{{ $t('val_personal_contribution') }}</div>
            </div>
                ...
                <van-list v-model:loading="loading" :finished="finished" :loading-text="$t('loading')" :finished-text="$t('no_more')"
                    ...
                    <div v-for="(record, index) in valRecords" :key="index" class="record-item">
                        <div class="wallet-address">
                            <span class="address-text" @click="router.push(`/valcontribution-details/${record.address}`)">{{ getStr(record.address, 6, 6) }}</span>
                            <img src="@/assets/imgs/identitycasting/copy.png" alt="" class="copy-icon" @click="copyAddress(record.address)">
                        </div>
                        <div class="contribution-value F-Bold">{{ record.contribution }}</div>
                    </div>
                </van-list>
            </div>

        ...
        <van-dialog v-model:show="show" class="dialog-box" :z-index="100" :showConfirmButton="false"
            ...
            <template #default>
                <div class="header plr-1">
                    <div class="title-line d-flex justify-content-between align-items-center">
                        <div class="title">
                            ...
                            <span class="mr-2">{{ t('supernode_destroy') }}</span>
                        </div>
                ...
                <div :class="['btn-box btn-item', isBalance ? '' : 'disabled-btn']" @click="handleClick">
                    <p>{{ isAllowance ? t('supernode_destroy') : t('supernode_authorize') }}</p>
                </div>
            </template>
        </van-dialog>
    </div>
...
import { formatDecimal, showLoading, closeToastLoading, showToastIcon } from '@/utils'
import { buyBadge, conflate, buySuperNode } from '@/api'
...
const handleClick = async () => {
    ...
    if (!isAllowance.value) {
        ...
        if (!code) {
            show.value = false;
            showToastIcon(t('destroySuccess'), 'success')
            setTimeout(() => {
                emit('finish');
            }, 2500)

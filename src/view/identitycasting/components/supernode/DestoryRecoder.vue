<template>
          ...
          <div class="default-box">
            <div class="title-line d-flex justify-content-between align-items-center">
              <div class="title">
                ...
                <span>{{ t('supernode_destroy_record') }}</span>
              </div>
              <img class="close-img" src="@/assets/imgs/identitycasting/close.png" alt="" @click="handleClickClose" />
            </div>
            <!-- 记录列表 -->
            <van-list v-if="recordList.length" v-model:loading="loading" :finished="finished" :finished-text="''"
              ...
              <div v-if="recordList.length > 0" class="record-list">
                <div class="record-item" v-for="(item, index) in recordList" :key="index">
                  <div class="item-line">
                    <div class="money F-Bold">
                      <span class="value"><i >-</i>
                        {{ getResultData(item.fc) }}</span>
                      <span class="unit">&nbsp;FC</span>
                    </div>
                    <div :class="['status-text', item.status === 0 ? 'color-gree' : '']">
                      {{item.status === 0 ? t('supernode_pending_confirm') : t('supernode_got_bonus_qualification')}}
                    </div>
                  ...
                  <div class="describe F-Regular accelerate-address" v-if="item.from_address">
                       {{ t('supernode_accelerate_address') }} : {{getStr(item.from_address, 6, 6) }}
                  </div>
            ...
            <div class="no-record" v-if="!recordList.length && finished">
              ...
              <div class="empty-text">{{ t('supernode_no_content') }}</div>
            </div>
  
            <div v-if="!recordList.length && !finished" class="no-record loading_text">
              <p>{{ t('supernode_loading') }}</p>
            </div>
  ...
  import { buySuperNodeList} from "@/api";
  ...
  const getRecordListData = async () => {
    const tempApi = buySuperNodeList;
    const { code, data, total } = await tempApi();
    if (!code && data) {
      loading.value = false;
